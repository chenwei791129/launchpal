# log-path-customization Specification

## Purpose

Users configure the default stdout/stderr log directories for new user services and new system daemons via the Settings page. The Log Storage section renders independent controls for User Log Directory and System Log Directory, each with Save and Reset to Default actions; saves go through `UpdateSettings`, surface inline validation errors verbatim, and refresh the locally cached Settings on success. The New Service modal sources its log path defaults from these settings as `<dir>/<label>/<stream>.log` and re-reads them every time it is opened so changes take effect without an application restart. Existing services are never migrated — saving Settings does not edit any plist or invoke any privileged-helper RPC. The system-domain log path allowlist is defined in exactly one Go package so the settings validator and the privileged helper can never drift.
