...
// Backs the Apple System page; filters identically to the other two surfaces.
...
async function loadServices() {
  ...
  try {
    services.value = await props.listFn()
...
async function checkPermissions() {
  try {
    if (window.go?.main?.App?.CheckPermissions) {
      ...
      hasPermission.value = perms[props.permissionKey] ?? true
    }
  } catch (e) {
