...
- `ServiceLogs.vue` renders a persistent info row between the controls row and the transient feedback row, showing `<logType> · <resolved path> · <size>` for the currently selected stream. It is present for **all three** service types.
- Data source is `GetLogClearStatus` — no separate binding. `launchctl.LogClearStatus` carries `Size int64` (json `size`) alongside `LogPath` / `Exists` / `UserWritable`. `logClearStatusFor` (`internal/launchctl/types.go`, shared by `readonly.go` and `user.go`) populates it from `f.Stat()` on the **same descriptor** the `O_WRONLY|O_NOFOLLOW` writability probe opened — never a separate `os.Stat`, which would reopen the race the single-open design avoids. `Size` is 0 whenever no descriptor was obtained (empty path, ENOENT, EACCES/ELOOP/EISDIR) **and** for a genuinely empty file; `Exists` is what tells those apart.
- The apple-system early return in `loadLogClearStatus` is gone — the query now runs for every domain. The Clear button stays hidden there because `clearControlState` short-circuits on `serviceType === 'apple-system'` before it ever consults the status.
- Rendering is driven by a `logInfo` computed with three states: `pending` (status still null — first load in flight, or the development fallback with no Wails bindings; renders `<logType> · Loading...` rather than claiming no path is configured), `no-path` (renders `No <stdout|stderr> path configured`, size segment omitted entirely), and `ready` (path + size).
- `formatLogSize` is base-1024, integer for `B` and one decimal above (`0 B` / `512 B` / `1.0 KB` / `2.4 MB` / `1.1 GB`), capped at `GB`; non-finite and negative inputs clamp to `0 B`. When `exists` is false the size renders as an em dash `—`, not `0 B` — `0 B` would claim the file exists and is empty.
- `truncatePathMiddle` shortens over-budget paths (`PATH_MAX_LEN = 72` characters, a stand-in for measuring rendered width) biased toward the tail so the basename survives; the unabbreviated path is always in the element's `title` tooltip.
- **The info row owns no timer.** It refreshes only alongside a log-content refresh: mount, stdout/stderr switch, manual Refresh, and — added with this feature — each Auto-refresh poll tick, where `startPolling` calls `loadLogClearStatus()` next to `loadLogs()`. A size pinned to the first load would read as stale beside a live-updating log body.
